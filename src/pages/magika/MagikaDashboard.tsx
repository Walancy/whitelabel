...
import {
  ChevronDown, ArrowUpRight, ChevronRight, RotateCcw, EyeOff,
...
const ArcGauge = ({ accentHex }: { accentHex: string }) => {
  ...
  return (
    <svg viewBox="0 0 200 106" className="w-full max-w-[190px] mx-auto">
...
const MetricCard = ({ label, value, sub, change, up, icon, accentHex, cardStyle }: MetricCardProps) => (
  <div className="rounded-2xl p-3 flex flex-col gap-1.5 flex-1 min-w-0" style={cardStyle}>
    <div className="flex items-center justify-between">
      <p className="text-[10px] text-muted-foreground font-medium">{label}</p>
      <div className="w-6 h-6 rounded-lg flex items-center justify-center" style={{ backgroundColor: `${accentHex}20` }}>
        <span style={{ color: accentHex }}>{icon}</span>
      </div>
    </div>
    <p className="text-base font-semibold text-foreground tracking-tight leading-none">{value}</p>
    <div className="flex items-center gap-1.5">
      <span className="text-[9px] text-muted-foreground">{sub}</span>
      <span className={cn('text-[9px] font-semibold flex items-center gap-0.5', up ? 'text-green-400' : 'text-red-400')}>
        {up ? <TrendingUp size={8} /> : <TrendingDown size={8} />}{change}
      </span>
    </div>
  </div>
);
...
const CryptoCard = ({ symbol, name, iconBg, icon, balance, price, change, positive, extra, chartData, chartColor, cardStyle }: CryptoCardProps) => (
  <div className="flex flex-col rounded-2xl p-3 gap-1.5 h-full" style={cardStyle}>
    <div className="flex items-start justify-between">
      <div className="flex items-center gap-2">
        ...
        <div>
          <p className="text-[9px] text-muted-foreground leading-none mb-0.5">{symbol}</p>
          <p className="text-[11px] font-semibold text-foreground leading-none">{name}</p>
        </div>
      </div>
      <span className={cn('text-[9px] font-semibold px-1.5 py-0.5 rounded-full flex items-center gap-0.5',
        positive ? 'bg-green-500/15 text-green-400' : 'bg-red-500/15 text-red-400')}>
        {positive ? <ArrowUpRight size={8} /> : null}{change}
      </span>
    </div>
    <div>
      <p className="text-base font-semibold text-foreground tracking-tight leading-tight">{balance}</p>
      <p className="text-[10px] text-muted-foreground">{price}</p>
    </div>
    <div className="h-7 w-full"><Sparkline data={chartData} color={chartColor} /></div>
    <p className="text-[10px] font-semibold text-green-400">{extra}</p>
  </div>
);
...
const ProfileChart = ({ accentHex }: { accentHex: string }) => {
  ...
  return (
    <div className="flex flex-col h-full gap-1.5 overflow-hidden min-h-0">
      <div className="flex items-center gap-0.5 flex-wrap shrink-0">
        {tabs.map(t => (
          <button key={t} onClick={() => setTab(t)}
            className={cn('px-2 py-1 rounded-lg text-[9px] font-semibold transition-all !h-auto',
      ...
      <div className="flex-1 min-h-0 flex gap-1.5 overflow-hidden">
        <div className="flex flex-col justify-between text-[8px] text-muted-foreground shrink-0 pb-4">
        ...
        <div className="flex-1 min-w-0 min-h-0 flex flex-col overflow-hidden">
          ...
          <div className="flex justify-between text-[8px] text-muted-foreground pt-0.5 shrink-0">
      ...
      <div className="flex flex-wrap items-center gap-x-2.5 gap-y-0.5 shrink-0">
        {[
          ...
          <div key={l.label} className="flex items-center gap-0.5 text-[8px] text-muted-foreground">
...
const TokenRow = ({ t }: { t: Token }) => (
  <div className="flex items-center gap-2 py-1.5 cursor-pointer hover:bg-white/5 px-2 -mx-2 rounded-xl transition-colors group">
    ...
    <div className="flex-1 min-w-0">
      <p className="text-[10px] font-semibold text-foreground leading-tight">{t.name}</p>
      <p className="text-[8px] text-muted-foreground">{t.symbol}</p>
    </div>
    <div className="w-12 h-5 shrink-0">
      <Sparkline data={t.spark} color={t.up ? '#22c55e' : '#ef4444'} />
    </div>
    <div className="text-right shrink-0">
      <p className="text-[10px] font-semibold text-foreground">{t.price}</p>
      <p className={cn('text-[8px] font-semibold', t.up ? 'text-green-400' : 'text-red-400')}>{t.change}</p>
    </div>
  </div>
);
...
const ActivityRow = ({ a }: { a: ActivityItem }) => (
  <div className="flex items-center gap-2 py-1.5">
    <div className={cn('w-5 h-5 rounded-full flex items-center justify-center shrink-0',
      ...
      {a.type === 'buy' ? <TrendingUp size={9} className="text-green-400" /> :
       a.type === 'sell' ? <TrendingDown size={9} className="text-red-400" /> :
       <Activity size={9} className="text-blue-400" />}
    </div>
    <div className="flex-1 min-w-0">
      <p className="text-[10px] font-semibold text-foreground capitalize leading-tight">{a.type} {a.coin}</p>
      <p className="text-[8px] text-muted-foreground">{a.time}</p>
    </div>
    <div className="text-right shrink-0">
      <p className="text-[10px] font-semibold text-foreground">{a.amount}</p>
      <p className="text-[8px] text-muted-foreground">{a.usd}</p>
    </div>
  </div>
);
  ...
  return (
    <div className="flex flex-col gap-2 w-full h-full">
      ...
      <div className="flex items-center justify-between shrink-0">
        <div className="flex items-center gap-2">
          <div>
            <p className="text-[9px] text-muted-foreground">◷ All Assets</p>
            <h1 className="text-sm font-semibold text-foreground tracking-tight">My Balance</h1>
          </div>
          <button className="flex items-center gap-1 text-[9px] font-semibold bg-card px-2 py-1 rounded-lg !h-auto text-foreground" style={cardStyle}>
            24h <ChevronDown size={9} />
          </button>
          <button className="w-6 h-6 flex items-center justify-center rounded-lg !h-auto" style={cardStyle}>
        ...
        <div className="flex items-center gap-2">
          <div>
            <p className="text-[9px] text-muted-foreground">◷ All Assets</p>
            <h2 className="text-sm font-semibold text-foreground tracking-tight">My Top Coins</h2>
          </div>
          <button className="flex items-center gap-1 text-[9px] font-semibold px-2 py-1 rounded-lg !h-auto text-foreground" style={cardStyle}>
            All Currencies <ChevronDown size={9} />
          </button>
          <button className="flex items-center gap-1 text-[9px] font-semibold px-2 py-1 rounded-lg !h-auto text-white" style={{ backgroundColor: accentHex }}>
            24h <ChevronDown size={9} />
          </button>
        </div>
      </div>
      ...
      <div className="flex gap-2 shrink-0">
        <MetricCard label="Portfolio Value"  value="$380,005" sub="Total balance" change="+5.2%" up icon={<Wallet size={12}/>}      accentHex={accentHex} cardStyle={cardStyle} />
        <MetricCard label="24h PnL"          value="+$2,322"  sub="vs. yesterday" change="+5.03%" up icon={<BarChart2 size={12}/>}   accentHex={accentHex} cardStyle={cardStyle} />
        <MetricCard label="Total Invested"   value="$361,200" sub="All time"      change="+2.1%" up icon={<DollarSign size={12}/>}   accentHex={accentHex} cardStyle={cardStyle} />
        <MetricCard label="ROI"              value="5.23%"    sub="Return on inv." change="+0.8%" up icon={<Percent size={12}/>}     accentHex={accentHex} cardStyle={cardStyle} />
        <MetricCard label="Fear & Greed"     value="72 / 100" sub="Market mood"   change="Greed" up icon={<Activity size={12}/>}    accentHex={accentHex} cardStyle={cardStyle} />
      </div>

      {/* ── Row 2: Crypto Cards + Arc ── */}
      <div className="grid grid-cols-12 gap-2 shrink-0" style={{ height: '150px' }}>
        ...
        {/* Center: Arc */}
        <div className="col-span-6 rounded-2xl px-4 py-2 flex flex-col items-center justify-center gap-0.5" style={cardStyle}>
          <p className="text-lg font-semibold text-foreground tracking-tight">$380,005.00</p>
          <div className="flex items-center gap-1">
            <span className="w-3 h-3 rounded-full bg-[#f97316] flex items-center justify-center text-white text-[7px]">↑</span>
            <span className="text-[9px] font-semibold text-green-400">+$2322.25</span>
            <span className="text-[9px] text-muted-foreground">(5.03)</span>
          </div>
          <div className="relative flex flex-col items-center w-full">
            <ArcGauge accentHex={accentHex} />
            <p className="text-[9px] font-semibold text-foreground -mt-2">7.52% More than last week</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-12 gap-2 flex-1 min-h-0 overflow-hidden">
        ...
        {/* Profile Chart */}
        <div className="col-span-5 rounded-2xl p-3 flex flex-col gap-1.5 overflow-hidden min-h-0" style={cardStyle}>
          <div className="flex items-center justify-between shrink-0">
            <p className="text-xs font-semibold text-foreground">Profile Chart</p>
            <button className="text-[9px] text-muted-foreground hover:text-foreground flex items-center gap-0.5 !h-auto">
        ...
        {/* Top Token */}
        <div className="col-span-4 rounded-2xl p-3 flex flex-col gap-1.5 overflow-hidden min-h-0" style={cardStyle}>
          <div className="flex items-center justify-between shrink-0">
            <p className="text-xs font-semibold text-foreground">Top Token</p>
            <button className="text-[9px] text-muted-foreground hover:text-foreground flex items-center gap-0.5 !h-auto">
              See all <ArrowUpRight size={10} />
            </button>
          </div>
          <div className="flex items-center gap-1 shrink-0">
            {(['Buy Crypto', 'By Exchange', 'Via API'] as const).map(t => (
              <button key={t} onClick={() => setFilterTab(t)}
                className={cn('px-2 py-0.5 rounded-full text-[8px] font-semibold transition-all !h-auto',
        ...
        {/* Recent Activity */}
        <div className="col-span-3 rounded-2xl p-3 flex flex-col gap-1.5 overflow-hidden min-h-0" style={cardStyle}>
          <div className="flex items-center justify-between shrink-0">
          ...
          <div className="shrink-0 pt-1">
            <p className="text-[8px] text-muted-foreground mb-1">Portfolio Allocation</p>
            <div className="flex flex-wrap gap-x-2 gap-y-0.5 mb-1">
              {portfolioAlloc.map(p => (
                <div key={p.label} className="flex items-center gap-1 text-[8px] text-muted-foreground">
