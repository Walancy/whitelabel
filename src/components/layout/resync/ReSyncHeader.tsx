import { Search, Bell, Grid, Plus, Sun, Moon, Link } from 'lucide-react';
import { useTheme, useChromeStyle } from '@/context/ThemeContext';

export const ReSyncHeader = () => {
    ...
    <header className="h-16 px-6 flex items-center justify-between border-b border-border sticky top-0 z-40 font-sans transition-colors duration-300" style={chromeStyle}>
      ...
      <div className="flex items-center gap-2">
        <button className="flex items-center gap-2 bg-primary text-primary-foreground px-4 py-2 rounded-lg text-xs font-semibold shadow-sm hover:brightness-110 active:scale-95 transition-all mr-4">
          <Plus size={16} className="text-primary-foreground" /> Add New
        </button>
        ...
        <button className="w-9 h-9 flex items-center justify-center rounded-lg hover:bg-accent transition-all border border-border bg-background group">
          <Link size={18} className="dark:text-foreground group-hover:text-foreground" />
        </button>

        <div className="flex items-center gap-2 px-2 border-l border-border ml-2 group cursor-pointer transition-all">
           <div className="w-9 h-9 rounded-lg bg-accent border border-border flex items-center justify-center overflow-hidden shrink-0 group-hover:border-border transition-all">
              <img 
