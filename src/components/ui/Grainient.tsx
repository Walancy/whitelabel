...
export default function Grainient({
    ...
    useEffect(() => {
        ...
        const renderer = new Renderer({ webgl: 2, alpha: true, antialias: false, dpr: Math.min(window.devicePixelRatio || 1, 1.5) } as ConstructorParameters<typeof Renderer>[0]);
        ...
        let raf = 0, lastTime = 0, running = true;
    ...
    // Update all uniforms reactively without recompiling the shader
    useEffect(() => {
        const p = programRef.current;
        if (!p) return;
        p.uniforms.uTimeSpeed.value = timeSpeed;
        p.uniforms.uColorBalance.value = colorBalance;
        p.uniforms.uWarpStrength.value = warpStrength;
        p.uniforms.uWarpFrequency.value = warpFrequency;
        p.uniforms.uWarpSpeed.value = warpSpeed;
        p.uniforms.uWarpAmplitude.value = warpAmplitude;
        p.uniforms.uBlendAngle.value = blendAngle;
        p.uniforms.uBlendSoftness.value = blendSoftness;
        p.uniforms.uRotationAmount.value = rotationAmount;
        p.uniforms.uNoiseScale.value = noiseScale;
        p.uniforms.uGrainAmount.value = grainAmount;
        p.uniforms.uGrainScale.value = grainScale;
        p.uniforms.uGrainAnimated.value = grainAnimated ? 1 : 0;
        p.uniforms.uContrast.value = contrast;
        p.uniforms.uGamma.value = gamma;
        p.uniforms.uSaturation.value = saturation;
        (p.uniforms.uCenterOffset.value as Float32Array).set([centerX, centerY]);
        p.uniforms.uZoom.value = zoom;
    ...
    useEffect(() => {
        const p = programRef.current;
        if (!p) return;
        (p.uniforms.uColor1.value as Float32Array).set(hexToRgb(color1));
        (p.uniforms.uColor2.value as Float32Array).set(hexToRgb(color2));
        (p.uniforms.uColor3.value as Float32Array).set(hexToRgb(color3));
