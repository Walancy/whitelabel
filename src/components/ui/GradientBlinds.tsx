...
export default function GradientBlinds({
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      const maxByMin = blindMinWidth > 0 ? Math.max(1, Math.floor(rect.width / blindMinWidth)) : blindCount;
    ...
    const loop = (t: number) => {
      ...
      if (!paused && meshRef.current) renderer.render({ scene: meshRef.current });
    };
    rafRef.current = requestAnimationFrame(loop);
